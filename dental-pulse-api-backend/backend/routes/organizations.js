...
const dentallyNameCache = new Map(); // api_key -> { name, ts }
 ...
 */
async function resolveDentallyAccountName(apiKey, apiEndpoint) {
  if (!apiKey || !apiEndpoint) return null;

  const cached = dentallyNameCache.get(apiKey);
  ...
  try {
    ...
    const names = (records || [])
      ...
      .filter(Boolean);
    // One site -> its name. Multiple sites -> primary name + "+N more" so the
    // dropdown stays compact but still signals a multi-site practice.
    let name = null;
    if (names.length === 1) name = names[0];
    else if (names.length > 1) name = `${names[0]} +${names.length - 1} more`;

    dentallyNameCache.set(apiKey, { name, ts: Date.now() });
    return name;
  } catch (err) {
    console.error(`[Organizations] Failed to resolve Dentally account name: ${err.message}`);
    // Cache the miss briefly so a broken/rate-limited key doesn't get retried
    // on every render within the same window.
    dentallyNameCache.set(apiKey, { name: null, ts: Date.now() });
    return null;
  }
}
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const { data: integrations } = await supabaseAdmin
      .from('integrations')
      .select('id, integration_name, is_connected, api_key, api_endpoints, sync_frequency, sync_at, created_at')
      .eq('organization_id', id)
      .is('deleted_at', null);
...
// PUT /api/organizations/:id/integration - Save/update Dentally API key and auto-trigger sync
router.put('/:id/integration', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const { api_key, api_endpoints } = req.body;

    if (!api_key || !api_endpoints) {
      return res.status(400).json({ error: 'api_key and api_endpoints are required' });
    }
    ...
    if (existing) {
      const { data, error } = await supabaseAdmin
        .from('integrations')
        .update({ api_key, api_endpoints, is_connected: true })
        .eq('id', existing.id)
        .select()
        .single();
      if (error) return res.status(500).json({ error: error.message });
  ...
  try {
    ...
    const accounts = await Promise.all(
      (integrations || []).map(async (i) => ({
        id: i.id,
        keyPrefix: i.api_key ? i.api_key.substring(0, 8) : null,
        name: await resolveDentallyAccountName(i.api_key, i.api_endpoints),
      }))
    );
