/**
 * Sage Other Receipts Service
 *
 * Bank-side receipts NOT from a contact (interest, transfers in).
 * Source: /other_receipts?attributes=all → sage_other_receipts table.
 */
...
async function syncAllOtherReceipts(organizationId, userId = null, connectionId = null) {
  ...
  while (hasNextPage) {
    console.log(`[SageOtherReceiptService] Fetching other_receipts page ${page}...`);

    let response;
    try {
      response = await sageClient.sageGet(integration, '/other_receipts', {
        attributes: 'all', items_per_page: ITEMS_PER_PAGE, page,
      });
    } catch (err) {
      if (err && /404|PathNotFound/i.test(err.message)) {
        console.warn('[SageOtherReceiptService] /other_receipts not available — skipping');
    ...
    for (const raw of items) {
      const row = transformRecord('sage_other_receipts', raw, ctx);
    ...
    for (let i = 0; i < rows.length; i += UPSERT_BATCH) {
      ...
      const { error: upsertErr, count } = await supabaseAdmin
        .from('sage_other_receipts')
        .upsert(batch, {
          onConflict: 'organization_id,platform_integration_id,sage_other_receipt_id',
          ignoreDuplicates: false,
          count: 'exact',
        });
      if (upsertErr) throw new Error(`Failed to upsert other_receipts batch: ${upsertErr.message}`);
  ...
  console.log(`[SageOtherReceiptService] Sync complete: ${totalUpserted} upserted (${totalFetched} fetched)`);
  return { upserted: totalUpserted, total_fetched: totalFetched, total_pages: totalPages };
}

module.exports = { syncAllOtherReceipts };
