/**
 * Sage Bank Transfers Service
 *
 * Transfers between bank accounts (e.g. moving money from Current → Savings).
 * Source: /bank_transfers?attributes=all → sage_bank_transfers table.
 */
...
async function syncAllBankTransfers(organizationId, userId = null, connectionId = null) {
  ...
  while (hasNextPage) {
    console.log(`[SageBankTransferService] Fetching bank_transfers page ${page}...`);

    let response;
    try {
      response = await sageClient.sageGet(integration, '/bank_transfers', {
        attributes: 'all', items_per_page: ITEMS_PER_PAGE, page,
      });
    } catch (err) {
      if (err && /404|PathNotFound/i.test(err.message)) {
        console.warn('[SageBankTransferService] /bank_transfers not available — skipping');
    ...
    for (const raw of items) {
      const row = transformRecord('sage_bank_transfers', raw, ctx);
    ...
    for (let i = 0; i < rows.length; i += UPSERT_BATCH) {
      ...
      const { error: upsertErr, count } = await supabaseAdmin
        .from('sage_bank_transfers')
        .upsert(batch, {
          onConflict: 'organization_id,platform_integration_id,sage_bank_transfer_id',
          ignoreDuplicates: false,
          count: 'exact',
        });
      if (upsertErr) throw new Error(`Failed to upsert bank_transfers batch: ${upsertErr.message}`);
  ...
  console.log(`[SageBankTransferService] Sync complete: ${totalUpserted} upserted (${totalFetched} fetched)`);
  return { upserted: totalUpserted, total_fetched: totalFetched, total_pages: totalPages };
}

module.exports = { syncAllBankTransfers };
