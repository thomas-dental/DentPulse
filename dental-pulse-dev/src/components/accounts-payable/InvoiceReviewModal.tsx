...
export function InvoiceReviewModal({
  ...
  // Check if selected location is mapped to a platform organization
  const { data: locationMapping, isLoading: isLoadingMapping } = useQuery({
    queryKey: ['location-org-mapping-review', effectiveLocationId, organizationId],
    queryFn: async () => {
      ...
      return {
        // platformOrgId is the platform_integration_organizations row id —
        // it matches xero_chart_of_accounts.xero_tenant_id and
        // quickbooks_chart_of_accounts.quickbooks_company_id.
        platformOrgId: platformOrgData.id,
        // platformOrgGuid is the PROVIDER's own org/business id (platform_org_id).
        // Sage's sage_chart_of_accounts.sage_business_id stores this GUID (not the
        // PIO row id), so the Sage COA scope must use this, not platformOrgId.
        platformOrgGuid: platformOrgData.platform_org_id,
  ...
  // Fetch Chart of Accounts based on platform type (xero or iplicit) and location mapping
  const { data: chartOfAccounts = [], isLoading: isLoadingCOA } = useQuery({
    queryKey: ['chart-of-accounts-review', organizationId, locationMapping?.platformOrgId],
    queryFn: async () => {
      if (!organizationId) return [];

      // Resolve platform / integration / company from the SELECTED location's
      // mapping, so the dropdown shows only that connected company's accounts.
      // (The old code grabbed the first integration in the org, which leaked
      // every company's chart of accounts into every location.)
      let platformName = locationMapping?.platformName?.toLowerCase();
      let integrationId: string | null = locationMapping?.platformIntegrationId || null;

      // Fallback: when the location is not mapped, use the first connected
      // accounting integration in the org.
      if (!platformName) {
      ...
      if (platformName === 'iplicit') {
        ...
        let iplicitQuery = (supabase as any)
          ...
          .eq('is_active', true)
          .eq('ap_flag', true);
        // Scope to the connection mapped to this location (iplicit has no
        // per-company column, so platform_integration_id is the finest scope).
        if (integrationId) {
        ...
        // Filter by platform_integration_id when available
        if (integrationId) {
        ...
        // quickbooks_company_id is the platform_integration_organizations row id.
        if (locationMapping?.platformOrgId) {
          quickbooksQuery = quickbooksQuery.eq('quickbooks_company_id', locationMapping.platformOrgId);
        }
        ...
        // mapped to this location (xero_tenant_id = platformOrgId).
        const buildXeroQuery = (scopeToTenant: boolean, scopeToIntegration: boolean) => {
          let q = (supabase as any)
            ...
            .eq('organization_id', organizationId)
            .eq('is_active', true);
          if (scopeToIntegration && integrationId) {
            q = q.eq('platform_integration_id', integrationId);
          }
          if (scopeToTenant && locationMapping?.platformOrgId) {
            q = q.eq('xero_tenant_id', locationMapping.platformOrgId);
          }
        ...
        // Fetch from sage_chart_of_accounts (dedicated Sage COA table).
        // sage_account_id = Sage ledger account GUID (used as ledger_account_id on push).
        ...
        // Scope to the location's specific Sage business — sage_business_id is
        // the Sage business GUID (platformOrgGuid), NOT the PIO row id (platformOrgId).
        if (locationMapping?.platformOrgGuid) {
          sageQuery = sageQuery.eq('sage_business_id', locationMapping.platformOrgGuid);
        }
    ...
    // When a location is selected, wait for its mapping to resolve so the
    // first fetch is already scoped to the right company.
  ...
  // renders instead of "Select Account".
  ...
  const { data: outOfScopeCoaLabels } = useQuery({
    queryKey: ['coa-fallback-labels-review', locationMapping?.platformName, savedLineItemAccountIds.sort().join(',')],
    queryFn: () => fetchCoaLabelsByIds(locationMapping?.platformName, savedLineItemAccountIds),
            ...
            {invoice.pdf_path && (
              <Button
                ...
                onClick={() => {
                  ...
                  if (pdfPath.startsWith('AP-Invoices/')) {
                    const pathParts = pdfPath.split('/');
                    const filename = pathParts[pathParts.length - 1];
                    const backendUrl = 'https://dent-enterprise-api.dentpulse.com';
                    window.open(`${backendUrl}/api/inbound-email-webhook/view-pdf/${filename}`, '_blank');
                  } else if (pdfPath.startsWith('http')) {
                    // Full URL - open directly
                    window.open(pdfPath, '_blank');
                  } else {
                    // Supabase storage - get public URL
                    const bucketName = pdfPath.startsWith('invoices/') ? 'account-payable-attechments' : 'inbound-attechments';
                    const { data } = supabase.storage.from(bucketName).getPublicUrl(pdfPath);
                    if (data?.publicUrl) {
            ...
            <Accordion type="single" collapsible>
              <AccordionItem value="invoice-information" className="border rounded-lg">
                ...
                <div className="border rounded-lg divide-y">
                  ...
                  <FieldRow label="From Address" value={invoice.vendor_address} confidence={confidenceScores.vendor_address} isHighlighted />
                  ...
                  <Table className="min-w-full">
                    <TableHeader>
                      <TableRow className="bg-indigo-600 hover:bg-indigo-600">
                        ...
                        <TableHead className="text-white font-medium min-w-[160px]">Expense Account</TableHead>
                                  ...
                                  <span className="truncate">
                                    {lineCoaIds[item.id || index]
                                      ? (() => {
                                          ...
                                          if (coa) {
                                            // QuickBooks accounts have no account number — show name only
                                            // rather than "null - <name>".
                                            return coa.coa_account_code
                                              ? `${coa.coa_account_code} - ${coa.coa_account_name}`
                                              : coa.coa_account_name;
                                          }
                                          // Outside the scoped options — render the unscoped label.
