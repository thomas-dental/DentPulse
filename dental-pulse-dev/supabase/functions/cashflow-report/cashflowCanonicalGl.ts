...
/**
 * Prefer iplicit over xero when both have journal lines; used for cashflow-report
 * non–category-based GL and category COA metadata when canonical rows exist.
 */
// deno-lint-ignore no-explicit-any
export async function resolveActiveCanonicalFinanceSource(
  ...
  if (preferred) {
    const preferredRows = rows.filter(
      (s) => String(s.platform_integration_id ?? "").trim() === preferred
    );
    for (const s of preferredRows) {
