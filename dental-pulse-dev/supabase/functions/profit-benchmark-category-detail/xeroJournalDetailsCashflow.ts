...
/** Prefer net (ex-tax) for Profit P&L; fall back to gross only if net missing. */
function lineAmount(row: { gross_amount?: unknown; net_amount?: unknown }): number {
  const net = row.net_amount;
  if (net != null && net !== "" && !Number.isNaN(Number(net))) {
    return Number(net);
  }
  return Number(row.gross_amount) || 0;
}
 ...
 *
 * amount uses Xero journal net_amount when present (Profit P&L), else gross_amount.
...
export async function fetchXeroJournalDetailsLikePlBsForCashflowReport(
  ...
  let offset = 0;
  const trackingOpt = trackingOptionId ? String(trackingOptionId).trim() : "";
  const BASE_SELECT =
  ...
  while (true) {
    let q = supabase
      ...
      // Stable order required for .range() pagination — date-only order skips/duplicates tied rows.
      .order("journal_date", { ascending: true })
      .order("id", { ascending: true })
      .range(offset, offset + PAGE - 1);

    if (tenantOrgRowId) {
      q = q.eq("platform_integration_organization_id", tenantOrgRowId);
    }
    if (trackingOpt && includeTrackingCol) {
      q = q.contains("tracking_option_ids", [trackingOpt]);
    }
  ...
  console.log(
    `[xeroJournalDetailsCashflow] loaded ${out.length} journal detail lines ` +
      `(integration=${xeroIntegrationId}, tenantRow=${tenantOrgRowId || "all"}, trackingOption=${trackingOpt || "none"})`
  );
  return out;
}
...
export async function loadXeroBankAccountIdsForCashflow(
  ...
  const tenantKeys = new Set(
    [tenantOrgRowId, mappedLegalEntityId]
