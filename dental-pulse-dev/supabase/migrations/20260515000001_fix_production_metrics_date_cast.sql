-- Fix chart_get_production_metrics: single-day ranges (start = end) returned 0.
--
-- tpi_completed_at is TIMESTAMP; p_start_date / p_end_date are DATE.
-- `timestamp BETWEEN date AND date` coerces both bounds to midnight, so a
-- single-day range like ('2026-05-14','2026-05-14') matched only TPIs
-- completed at exactly 2026-05-14 00:00:00 — effectively zero.
--
-- chart_get_profit_metrics avoids this by casting the column to DATE first
-- (see get_provider_net_production_monthly). Apply the same cast here.
