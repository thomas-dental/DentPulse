-- Fast aggregate RPC for PE Invoices outstanding KPIs + aged-debt buckets.
-- Replaces full pe_invoices_mapped_rows fetch for aged-debt and hero KPI reads.

CREATE INDEX IF NOT EXISTS idx_pii_pe_dentally_outstanding_live
  ON public.platform_integration_invoices (organization_id, location_id)
  INCLUDE (due_date, invoice_date, subtotal, amount_outstanding, status, is_paid, patient_id)
  WHERE platform_type = 'dentally'
    AND deleted_at IS NULL
    AND LOWER(COALESCE(status, '')) <> 'voided'
    AND (
      COALESCE(amount_outstanding, 0) > 0
      OR (COALESCE(is_paid, false) IS NOT TRUE AND COALESCE(subtotal, 0) > 0)
    );
  ...
  RETURN (
    WITH inv AS MATERIALIZED (
      SELECT
        ...
        ROUND(
          CASE
            WHEN COALESCE(i.amount_outstanding, 0) > 0 THEN i.amount_outstanding
            WHEN COALESCE(i.is_paid, false) THEN 0::numeric
    ...
    with_plan AS (
      ...
      LEFT JOIN public.patients p
       ...
       AND p.pt_id = o.patient_id::bigint
    ...
    SELECT jsonb_build_object(
      'totalOutstandingGbp',
        ROUND(COALESCE((SELECT SUM(outstanding_gbp) FROM with_plan), 0)::numeric, 2),
      'overdue60PlusGbp',
        ROUND(
          COALESCE(
            (SELECT SUM(outstanding_gbp) FROM with_plan WHERE days_past_due > 60),
            0
          )::numeric,
          2
        ),
...
COMMENT ON FUNCTION public.pe_invoices_outstanding_kpis(
  ...
  'Outstanding invoice KPIs and aged-debt buckets for PE Invoices (no full row scan).';
