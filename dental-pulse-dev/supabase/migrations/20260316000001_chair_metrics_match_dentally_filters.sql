-- Fix: Chair metrics — match Dentally filters: apmt_patient_id IS NOT NULL, no state filter.
-- Also includes 4-tier location fallback to prevent NULL location drops.
...
BEGIN
  ...
  -- Default location for appointments with no location match
  ...
  -- Current period: patient appointments only (apmt_patient_id IS NOT NULL), no state filter
  -- 4-tier location fallback: apmt.location_id -> patient.location_id -> practitioner_site_id -> default
  -- Utilisation: appointment start to finish (apmt_start_time to apmt_finish_time)
  apmt_current AS (
    SELECT
      COALESCE(
        a.location_id,
  ...
  -- Occupancy: completed treatment duration from treatments.duration_minutes
  -- Chain: appointments → treatment_appointments → treatment_plan_items → treatments
  ...
  SELECT
    ...
    -- Occupancy: completed treatment duration (treatments.duration_minutes) vs available hours
    CASE
    ...
    -- Utilisation: appointment window (start to finish) vs available hours
    CASE
      WHEN l.loc_chairs > 0 AND (l.loc_chairs * l.loc_daily_hours * _working_days) > 0
