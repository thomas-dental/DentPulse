-- PE read scope: optional location + date range on summary RPCs.
  ...
  IF use_facts THEN
    ...
    WHERE f.practice_id = p_practice_id
      ...
      AND (
        ...
        OR (p.id IS NULL AND f.location_id = p_location_id)
      );
...
DROP FUNCTION IF EXISTS public.pe_invoice_contribution_summary(UUID);
...
DROP FUNCTION IF EXISTS public.pe_retention_segment_rollup(UUID, UUID);

GRANT EXECUTE ON FUNCTION public.pe_retention_segment_rollup(UUID, UUID, DATE, DATE)
  TO authenticated, service_role;
