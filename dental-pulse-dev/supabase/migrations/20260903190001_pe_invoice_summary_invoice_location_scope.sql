-- Scope PE invoice summary by invoice location (not patient home location).
-- Fixes NHS/UDA and revenue mix when TopBar location filter is active.
...
BEGIN
  ...
  IF use_facts THEN
    ...
    SELECT jsonb_build_object(
      ...
      'invoices_with_revenue', COUNT(*) FILTER (WHERE f.revenue_private_plan > 0)::bigint,
      'patient_count', COUNT(DISTINCT f.patient_id)::bigint,
      'patients_with_revenue', COUNT(DISTINCT f.patient_id) FILTER (WHERE f.revenue_private_plan > 0)::bigint,
      'total_contribution', COALESCE(SUM(f.contribution), 0),
      'total_revenue', COALESCE(SUM(f.revenue_private_plan), 0),
      'revenue_nhs', COALESCE(SUM(f.revenue_nhs), 0),
      'revenue_private', revenue_private,
      'revenue_plan', revenue_plan,
      'invoices_missing_practitioner', COUNT(*) FILTER (WHERE f.has_missing_practitioner)::bigint,
      'invoices_missing_rate', COUNT(*) FILTER (WHERE f.has_missing_rate)::bigint,
      'revenue_no_practitioner', COALESCE(SUM(f.revenue_no_practitioner), 0),
      'revenue_missing_rate', COALESCE(SUM(f.revenue_missing_rate), 0)
    )
    INTO result
    FROM public.pe_invoice_contribution_facts f
    LEFT JOIN public.platform_integration_invoices inv
...
COMMENT ON FUNCTION public.pe_invoice_contribution_summary(UUID, UUID, DATE, DATE) IS
  'PE invoice contribution + revenue mix. Paid invoices only. Location scope uses invoice location (fallback patient location).';
