-- Retention recovery loop: flags + patient worklist meta in one round-trip.

CREATE INDEX IF NOT EXISTS idx_appt_last_visit_worklist
  ON public.appointments (organization_id, apmt_patient_id, apmt_completed_at DESC)
  WHERE apmt_completed_at IS NOT NULL
    AND LOWER(BTRIM(COALESCE(apmt_state, ''))) NOT IN ('cancelled', 'did not attend', 'dna');
...
COMMENT ON FUNCTION public.pe_retention_recovery_loop(UUID, UUID) IS
  'Retention recovery loop read: reactivation flags enriched with patient meta and last visit in one query.';
