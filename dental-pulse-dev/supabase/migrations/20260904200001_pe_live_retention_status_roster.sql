-- Patient roster + retention rollups: read live retention from v_pe_retention_segment
-- instead of stale pe_patient_contribution_facts.retention_status.

CREATE OR REPLACE FUNCTION public.pe_live_retention_status(
  p_practice_id UUID,
  p_patient_id UUID
)
RETURNS TEXT
...
AS $$
  SELECT COALESCE(
    (
      SELECT CASE LOWER(BTRIM(rs.retention_status))
        WHEN 'healthy' THEN 'active'
        WHEN 'drifting' THEN 'drifting'
        WHEN 'lapsed' THEN 'lapsed'
        WHEN 'effectively_lost' THEN 'effectively_lost'
        ELSE 'active'
      END
      FROM public.v_pe_retention_segment rs
      WHERE rs.practice_id = p_practice_id
...
COMMENT ON FUNCTION public.pe_live_retention_status(UUID, UUID) IS
  'Live 4-tier retention for a patient (v_pe_retention_segment / pe_retention_status). Defaults active when not on file.';

GRANT EXECUTE ON FUNCTION public.pe_live_retention_status(UUID, UUID) TO authenticated, service_role;


CREATE OR REPLACE FUNCTION public.pe_normalize_retention_status(raw TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE LOWER(BTRIM(COALESCE(raw, 'active')))
    WHEN 'healthy' THEN 'active'
    WHEN 'drifting' THEN 'drifting'
    WHEN 'lapsed' THEN 'lapsed'
    WHEN 'effectively_lost' THEN 'effectively_lost'
...
GRANT EXECUTE ON FUNCTION public.pe_normalize_retention_status(TEXT) TO authenticated, service_role;
...
COMMENT ON FUNCTION public.pe_patient_roster_page IS
  'Paginated PE patient roster. Live retention from v_pe_retention_segment; ranks on facts/grain then enriches page rows.';
...
COMMENT ON FUNCTION public.pe_patient_roster_summary IS
  'PE patient roster aggregates; live retention from v_pe_retention_segment.';
...
COMMENT ON FUNCTION public.pe_retention_segment_rollup(UUID, UUID, DATE, DATE) IS
  'Retention segment rollup; live status from v_pe_retention_segment when facts exist.';
