...
-- 1. Add p_location_id parameter (location filter, same as other chart RPCs)
-- 2. Replace hardcoded /8.0 with open_hours_per_day from organizations
-- 3. Fix ARRAY_AGG(p.external_id) → ARRAY_AGG(p.external_id::TEXT) to avoid
--    type mismatch when external_id is integer type
