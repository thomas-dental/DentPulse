-- ============================================================================
-- Supersedes migration 000003.
--
-- Replace org-level platform detection with per-location platform routing.
-- Each location is mapped to a platform (xero / iplicit) via
-- platform_integration_organization_mapping → platform_integration_organizations.
--
-- Specific location  → look up its platform → call the right cost function.
-- All locations      → sum Xero-mapped locations + unique Iplicit legal entities.
-- ============================================================================
...
BEGIN
  ...
  -- ── Op Costs: route per location platform mapping ─────────────────────────
  IF p_location_id IS NOT NULL THEN

    -- Look up this location's mapped platform and legal-entity/tenant id
    SELECT pio.platform_name, pio.platform_org_id
    INTO v_location_platform, v_legal_entity_id
    ...
    IF v_location_platform = 'xero' THEN
      SELECT COALESCE(get_xero_op_cost(p_organization_id, p_location_id, p_start_date, p_end_date), 0)
      INTO v_op_costs;

    ELSIF v_location_platform = 'iplicit' THEN
      SELECT COALESCE(ABS(pl.total_amount), 0)
      INTO v_op_costs
      FROM get_iplicit_pl_amount_cost_by_date(
        p_organization_id, p_start_date, p_end_date, 'TC', v_legal_entity_id
      ) pl;

    ELSE
      -- Location unmapped or unknown platform
      v_op_costs := 0;
    END IF;

  ELSE

    -- All locations: sum Xero-mapped locations
    SELECT COALESCE(SUM(
      get_xero_op_cost(p_organization_id, xero_locs.loc_id, p_start_date, p_end_date)
    ), 0)
    INTO v_op_costs
    FROM (
      SELECT DISTINCT piom.location_id AS loc_id
      FROM platform_integration_organization_mapping piom
      JOIN platform_integration_organizations pio
        ON pio.id = piom.platform_integration_organizations_id
      WHERE piom.organization_id = p_organization_id
        AND pio.platform_name    = 'xero'
    ) xero_locs;

    -- All locations: add Iplicit costs per unique legal entity (avoids double-counting
    -- when multiple locations share the same legal entity)
    FOR v_row IN
      SELECT DISTINCT pio.platform_org_id
      FROM platform_integration_organization_mapping piom
      JOIN platform_integration_organizations pio
        ON pio.id = piom.platform_integration_organizations_id
      WHERE piom.organization_id = p_organization_id
    ...
    LOOP
      ...
      INTO v_iplicit_cost
      FROM get_iplicit_pl_amount_cost_by_date(
        p_organization_id, p_start_date, p_end_date, 'TC', v_row.platform_org_id
      ) pl;

      v_op_costs := v_op_costs + COALESCE(v_iplicit_cost, 0);
    END LOOP;
