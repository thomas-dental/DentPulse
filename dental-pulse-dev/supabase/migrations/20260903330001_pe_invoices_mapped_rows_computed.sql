-- Push outstanding £, aging, and leakage flags into pe_invoices_mapped_rows (one SQL pass).

DROP FUNCTION IF EXISTS public.pe_invoices_mapped_rows(UUID[], UUID, DATE, DATE);
...
COMMENT ON FUNCTION public.pe_invoices_mapped_rows(
  ...
  'Scoped invoice rows with JOINs plus outstanding £, aging bucket, and cash-leakage flags.';

GRANT EXECUTE ON FUNCTION public.pe_invoices_mapped_rows(
  UUID[], UUID, DATE, DATE, DATE, INT, INT, INT, INT
) TO service_role;
