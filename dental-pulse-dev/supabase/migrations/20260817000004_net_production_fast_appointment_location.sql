-- ============================================================================
-- Speed up appointment-site location lookup so wide Production Data ranges
-- (This Year / Jan–Jul) do not time out and render "No production data".
--
-- 20260817000002 joined treatment_appointments × appointments onto every TPI.
-- Postgres often hash-joined the whole appointments table; All Locations then
-- fired that query once per site in parallel and PostgREST timed out.
--
-- Fix: LATERAL LIMIT 1 nested-loop lookup (unique org+ta_id / org+apmt_id)
-- plus an appointments (organization_id, apmt_id) index.
-- ============================================================================

CREATE INDEX IF NOT EXISTS idx_appointments_org_apmt_id_active
  ON public.appointments (organization_id, apmt_id)
