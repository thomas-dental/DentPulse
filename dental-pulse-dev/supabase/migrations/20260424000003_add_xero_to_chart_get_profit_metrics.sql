-- ============================================================================
-- Add Xero op-cost branch to chart_get_profit_metrics.
--
-- Problem: the existing location-mapping guard
--   IF p_location_id IS NOT NULL AND NOT v_location_has_mapping THEN op_costs = 0
-- was written for Iplicit only. v_location_has_mapping is always FALSE for
-- Xero (the lookup filters on platform_name = 'iplicit'), so Xero locations
-- always returned £0 for Op Costs.
--
-- Fix: check platform first, then handle location filtering within each branch.
--   iplicit → existing logic (legal-entity mapping required)
--   xero    → call get_xero_op_cost per location; sum all locations when NULL
--   other   → 0
-- ============================================================================
...
BEGIN
  ...
  -- Determine which accounting platform is connected
  ...
  -- ── Iplicit: resolve legal entity from location mapping ──────────────────
  ...
  IF v_connected_platform = 'iplicit' THEN
    -- Location selected but no Iplicit mapping → can't attribute costs
  ...
  ELSIF v_connected_platform = 'xero' THEN
    -- Xero: account selection is stored per practice_location.
    ...
    IF p_location_id IS NOT NULL THEN
      ...
      INTO v_op_costs;
    ELSE
      -- All-locations view: sum across every practice_location for this org
