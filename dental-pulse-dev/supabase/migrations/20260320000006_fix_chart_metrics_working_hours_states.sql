-- Fix chart_get_production_metrics working hours calculation to match
-- get_provider_working_hours_by_location (used by Profit Goals Settings).
--
-- Root cause of Avg Daily Production mismatch (e.g. £7,250.37 vs £6,551.15):
-- The provider_hours CTE was using states (Completed, Pending, In surgery, Confirmed)
-- AND filtering apmt_patient_id IS NOT NULL, which EXCLUDES blocked/admin slots.
-- get_provider_working_hours_by_location uses (Completed, Pending, In surgery) with
-- NO patient_id filter, so it INCLUDES blocked slots — more hours → lower avg daily.
--
-- Fix: align provider_hours to use the same 3 states and remove patient_id filter.
...
BEGIN
  ...
  base_providers AS (
    ...
    WHERE
      ...
      -- The location filter is applied to the TPI and appointment data below.
  ...
  provider_hours AS (
    ...
    LEFT JOIN appointments a
      ...
      -- Match get_provider_working_hours_by_location: 3 states, no patient_id filter
