-- ============================================================================
-- Net production must follow Setup Categories → Profit (Revenue & Costs) →
-- Private Income payment plans, not Dentally's unfiltered practitioner total.
--
-- Bug (Appoline Dental Care / Zahid Hussain / Dec-25):
--   Dentally total production = £406. Setup Categories Private Income plans
--   produce £265. The Production Data table showed £406 because:
--     1. The RPC built the private-plan list from
--        practice_locations.provider_private_income_accounts only (org-wide),
--        ignoring the location's Setup Categories PMS mapping in
--        private_income_accounts.
--     2. chart_get_production_metrics summed every TPI with no plan filter.
--   The frontend remainder-fallback (useAllProvidersNetProduction) then
--   stuffed unclassified TPI into membership so the cell equalled Dentally.
--
-- Fix:
--   Shared helper returns the selected location's Private Income pp_ids
--   (provider_private_income_accounts ∪ private_income_accounts), expanded
--   by plan name. Both net-production RPCs and chart_get_production_metrics
--   use it. When a location is selected, only THAT location's mapping applies
--   — matching the Setup Categories UI. Unconfigured orgs still get NULL
--   (legacy: all TPI / remainder fallback).
-- ============================================================================
