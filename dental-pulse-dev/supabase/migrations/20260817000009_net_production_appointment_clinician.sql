-- ============================================================================
-- Robert Stone Aug-25: DentPulse £2,548 vs Dentally £2,696 (Enterprise TPI
-- practitioner total £2,623). Gap vs Dentally is £148 − £75 = £73.
--
-- Appoline Robert (299878) TPI = £2,623. A £75 Review has TPI 299878 but the
-- appointment was St Catherine's Robert (263849) — appointment-site drops it
-- from Appoline → £2,548. A £148 Perio Assessment has TPI 263849 but the
-- appointment clinician is Appoline Robert (299878). Location-scoped IDs
-- then ignore 263849 on the Appoline fetch, so £148 never lands on Robert.
--
-- Dentally Appoline Robert = 2623 − 75 + 148 = 2696 (appointment clinician).
-- Same pattern as Dimitra: TPI kept the other-site ID, appointment is this
-- site's clinician. Attribute production to
-- COALESCE(appointment.practitioner_id, tpi.practitioner_id). Keep the
-- same-person guard so a different dentist's appointment does not steal TPI
-- (Zahid retainer / practitioner 57217).
-- ============================================================================
...
BEGIN
  ...
  tpi_home AS (
    SELECT
      COALESCE(ap.apmt_practitioner_id, tpi.tpi_practitioner_id)::INTEGER AS ext_id,
...
BEGIN
  ...
  tpi_home AS (
    ...
    WHERE tpi.organization_id     = p_organization_id
      AND COALESCE(ap.apmt_practitioner_id, tpi.tpi_practitioner_id) = p_practitioner_id
      AND tpi.tpi_completed_at    IS NOT NULL
      AND tpi.tpi_completed       = true
      AND tpi.tpi_completed_at    >= v_from_ts
      AND tpi.tpi_completed_at    <  v_to_ts
