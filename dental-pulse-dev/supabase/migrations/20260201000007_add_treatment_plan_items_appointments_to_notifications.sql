-- Add treatment_plan_items and treatment_appointments to notification entity name mapping
-- This ensures proper display names in sync completion notifications
...
BEGIN
  ...
  IF (NEW.status = 'completed' OR NEW.status = 'failed') AND
    ...
    -- Format entity name (includes treatment_plan_items and treatment_appointments)
...
COMMENT ON FUNCTION create_sync_completion_notification() IS 'Creates notifications for sync job completions/failures. Includes treatment_plan_items and treatment_appointments. Prevents duplicates by checking for similar notifications within the last 5 minutes.';
