-- Fix: get_chair_metrics was inflating occupancy/utilisation > 100% due to row duplication.
--
-- Root causes:
-- 1. LEFT JOIN patients on apmt_patient_id multiplied rows when a patient had >1 row per org
--    (sync duplicates), inflating appointment_hours by the duplicate count.
-- 2. LEFT JOIN practice_locations on api_record_unique_id multiplied rows when multiple
--    locations shared the same Dentally site UUID.
-- 3. TPI dedup: treatment_plan_items may have duplicates when tpi_id is NULL (known issue —
--    parseBigIntTPI returns NULL for UUID ids, UNIQUE constraint fails because NULL != NULL).
--    Summing treatments.duration_minutes via JOIN then multiplies treatment_hours.
--
-- Fix strategy: use LATERAL subqueries with LIMIT 1 for the patient / location fallback
-- lookups (guarantees one row per appointment), and dedupe TPIs on a logical fingerprint
-- before joining to treatments for occupancy.
...
BEGIN
  ...
  -- Prefers tpi_id when set; falls back to (patient, completed_at, price, treatment, invoice)
  -- composite for UUID-based records with NULL tpi_id.
  tpi_dedup AS (
    SELECT DISTINCT ON (
      COALESCE(
        tpi.tpi_id::TEXT,
  ...
  SELECT
    ...
    CASE
      ...
      THEN ROUND(
        LEAST(COALESCE(oc.treatment_hours, 0), (l.loc_chairs * l.loc_daily_hours * _working_days))
        / (l.loc_chairs * l.loc_daily_hours * _working_days) * 100, 1)
      ELSE 0
    END AS occupancy_pct,
    CASE
      WHEN l.loc_chairs > 0 AND (l.loc_chairs * l.loc_daily_hours * _working_days) > 0
      THEN ROUND(
        LEAST(COALESCE(ac.appointment_hours, 0), (l.loc_chairs * l.loc_daily_hours * _working_days))
        / (l.loc_chairs * l.loc_daily_hours * _working_days) * 100, 1)
      ELSE 0
...
-- Apply same LATERAL-LIMIT fix to hourly RPC (same LEFT JOIN patients/locations bug).
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      THEN ROUND(
        LEAST(COALESCE(ab.total_minutes, 0), (_total_chairs * 60.0 * _working_days))
        / (_total_chairs * 60.0 * _working_days) * 100, 1)
      ELSE 0
    END AS utilisation_pct
  FROM hours hrs
