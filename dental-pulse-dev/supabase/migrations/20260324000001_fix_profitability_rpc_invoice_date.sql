-- Fix profitability RPC to match Dentally Income Report:
-- 1. Use invoice_date (dated_on) for date range filtering — matches Dentally's
--    Income report which filters by the invoice creation date, not the paid date
-- 2. Keep is_paid = true — only count revenue from paid invoices
-- 3. Simplify GROUP BY to prevent row splitting
-- 4. Include sundries + 0-value items
