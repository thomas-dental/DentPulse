-- ============================================================================
-- Patient Economics — v_invoice_contribution: default private_share_rate to 0
-- when a dominant practitioner exists but no rate row is configured.
-- Contribution then uses rate 0 (clinician_cost = 0, contribution = full private £).
-- has_missing_rate remains true so Settings can still surface the gap.
-- Missing practitioner still excludes contribution (no one to attribute to).
-- ============================================================================
...
invoice_resolved AS (
  SELECT
    ...
    -- Default unset rates to 0 when a practitioner is attributed
    CASE
...
SELECT
  ...
  CASE
    WHEN ir.private_revenue > 0 AND ir.dominant_provider_id IS NULL
  ...
  -- Informational: revenue that used the default rate 0 (still included in contribution)
  CASE
    WHEN ir.rate_was_defaulted THEN ir.private_revenue
    ELSE 0
  END::numeric(15, 2) AS revenue_missing_rate,
  CASE
    WHEN ir.dominant_provider_id IS NOT NULL
    THEN ROUND(ir.private_revenue * (ir.effective_rate / 100.0), 2)::numeric(15, 2)
    ELSE 0::numeric(15, 2)
  ...
  CASE
    ...
    THEN (
      ir.private_revenue
      - ROUND(ir.private_revenue * (ir.effective_rate / 100.0), 2)
    )::numeric(15, 2)
    ELSE 0::numeric(15, 2)
  END AS contribution,
  CASE
...
COMMENT ON VIEW public.v_invoice_contribution IS
  'PE invoice contribution: one row per Dentally invoice. Dominant practitioner × private_share rate (defaults to 0% when unset). Missing practitioner still excludes contribution. security_invoker for org RLS.';
