-- Planned > N days unscheduled leakage: one round-trip (ledger + plan items + patients).

CREATE INDEX IF NOT EXISTS idx_treatments_org_ext_private
  ON public.treatments (organization_id, external_id)
  WHERE deleted_at IS NULL AND treatment_type = 'private';
...
COMMENT ON FUNCTION public.pe_planned_unscheduled_leakage(UUID, UUID, DATE, DATE) IS
  'Planned unscheduled leakage: private plan items on open unscheduled plans past threshold days (one query).';
