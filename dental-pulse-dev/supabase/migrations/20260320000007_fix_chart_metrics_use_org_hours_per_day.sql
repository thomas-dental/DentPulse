-- Fix chart_get_production_metrics: use organizations.open_hours_per_day
-- instead of hardcoded 8 when converting appointment hours → working days.
--
-- Root cause: Profit Goals divides total_hours by open_hours_per_day (org setting),
-- but Overview Ranking always divided by 8. If the org has e.g. 9 hours/day, the
-- working_days denominator differs → avg_daily_production figures never match.
--
-- Fix: fetch open_hours_per_day from organizations, default 8 if null/0.
-- Also aligns appointment date filter to ::DATE comparison (matches
-- get_provider_working_hours_by_location and avoids midnight boundary exclusion).
...
BEGIN
  -- Fetch org-configured hours per day (defaults to 8 if not set)
