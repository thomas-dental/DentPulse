-- Update practitioner activity report function to increase statement timeout
-- This fixes the timeout issue when fetching 12 months of data
...
AS $$
BEGIN
  -- Increase statement timeout for this function to handle large date ranges
...
'Get practitioner activity report from treatment_plan_items (matches .NET procedure logic).
Filters: completed treatments only (tpi_completed = true AND tpi_completed_at IS NOT NULL).
Updated with 30s timeout for large date ranges.
