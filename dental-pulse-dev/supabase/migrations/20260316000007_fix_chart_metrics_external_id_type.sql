-- Fix: external_id is INTEGER, so ARRAY_AGG returns integer[] which conflicts with
-- the ARRAY[]::TEXT[] fallback in COALESCE. Cast external_id to TEXT in the aggregation
-- so the array is always text[] throughout, then cast back to INTEGER only where needed.
...
BEGIN
  ...
  base_providers AS (
    SELECT
      ...
      -- Cast external_id to TEXT so the array type is always text[]
      COALESCE(
...
BEGIN
  ...
  practice_pl_calcs AS (
    SELECT
      ...
      fc.working_days,
      fc.total_production - (
        (fc.assoc_gross_share - fc.cost_of_labs * (fc.lab_split_pct / 100.0)) +
        fc.cost_of_labs +
        fc.materials_costs +
...
BEGIN
  ...
  base_providers_with_planning AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE pdp.provider_id = p.id AND pdp.organization_id = p_organization_id
  ...
  SELECT
    pmd.provider_id,
    pmd.provider_name,
    ROUND(CASE WHEN pmd.total_hours > 0 THEN pmd.total_amount / (pmd.total_hours / 8.0) ELSE 0 END, 2) AS daily_production,
    pmd.planning_target AS planning_avg_daily_production,
    ROUND(CASE WHEN pmd.total_hours > 0 THEN pmd.total_amount / (pmd.total_hours / 8.0) ELSE 0 END - pmd.planning_target, 2) AS target_gap,
    CASE
