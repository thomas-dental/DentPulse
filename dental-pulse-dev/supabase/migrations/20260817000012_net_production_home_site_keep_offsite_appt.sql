-- ============================================================================
-- George Farchat Appoline Apr-26: DentPulse Net Production £5,668.30 vs
-- Dentally / Practitioner Activity £6,935.00. Gap = £1,266.70.
--
-- 00011 still dropped home-site TPI when the appointment was at another
-- site (tpi_home required ap.location_id = selected location). George
-- treated 7 items on 24 Apr 2026 on Appoline practitioner 189683, TPI
-- location Appoline, but the appointment was booked at Elegant Smiles
-- Stamford under his other-site id 278945. Dentally attributes by
-- treatment practitioner, so Appoline's Practitioner Activity keeps them.
--
-- Home-site IDs now keep their own TPI even when the appointment is
-- elsewhere. The Zahid guard still applies only to OTHER-SITE Dentally
-- IDs. Dimitra path (TPI stamped at another site, appointment here)
-- is unchanged.
-- ============================================================================
...
BEGIN
  ...
  -- Site-stamped TPI. Home-site practitioners keep their own TPI even when
  -- the appointment was booked at another site (George Farchat Appoline
  -- Apr-26: 7 items on a Stamford appointment, Appoline TPI practitioner).
  -- Other-site IDs still require the appointment at the selected location.
  tpi_home AS (
    SELECT
    ...
    WHERE tpi.organization_id     = p_organization_id
      ...
      AND (
        ...
        OR ap.location_id IS NULL
        OR ap.location_id = p_location_id
        OR EXISTS (
          SELECT 1 FROM providers_in_scope ps
          WHERE ps.ext_id = tpi.tpi_practitioner_id::INTEGER
        )
      )
      AND (
        EXISTS (
        ...
        OR ap.apmt_practitioner_id IS NULL
...
BEGIN
  ...
  home_practitioner AS (
    SELECT EXISTS (
      SELECT 1
      FROM providers p
      WHERE p.organization_id = p_organization_id
        AND p.deleted_at IS NULL
        AND p.external_id::INTEGER = p_practitioner_id
        AND (p_location_id IS NULL OR p.location_id = p_location_id)
    ) AS is_home
  ),
  ...
  tpi_home AS (
    ...
    WHERE tpi.organization_id     = p_organization_id
      ...
      AND (
        ...
        OR ap.location_id IS NULL
        OR ap.location_id = p_location_id
        OR (SELECT is_home FROM home_practitioner)
      )
      AND (
        (SELECT is_home FROM home_practitioner)
        OR ap.apmt_practitioner_id IS NULL
        OR ap.apmt_practitioner_id = tpi.tpi_practitioner_id
        OR EXISTS (
          SELECT 1 FROM same_person sp
