-- ============================================================================
-- Add QuickBooks op-cost branch to chart_get_profit_metrics.
--
-- Problem: the platform detection used org-level platform_integrations with
-- LIMIT 1, which is unreliable when multiple platforms are connected (e.g. an
-- org with both Xero and QuickBooks). A location mapped to QuickBooks would
-- incorrectly fall into the Xero branch and return £0.
--
-- Fix:
--   • When p_location_id IS NOT NULL — resolve platform from
--     platform_integration_organization_mapping → platform_integration_organizations
--     (the same per-location mapping used in the Settings page).
--     Fall back to org-level only when the location has no mapping.
--   • When p_location_id IS NULL — keep the existing org-level detection.
--   • Add quickbooks branch: calls get_quickbooks_op_cost per location;
--     sums all locations when p_location_id IS NULL.
-- ============================================================================
...
BEGIN
  ...
  -- ── Platform detection ────────────────────────────────────────────────────
  -- When a specific location is given, resolve the platform from its mapping
  -- (platform_integration_organization_mapping → platform_integration_organizations).
  -- This correctly handles orgs that have multiple platforms connected (e.g. Xero
  -- for one location, QuickBooks for another).
  -- For Iplicit, platform_org_id doubles as the legal entity id.
  ...
  -- Fall back to org-level detection when no per-location mapping exists
  -- (also used for the all-locations view where p_location_id IS NULL).
  ...
  IF v_connected_platform = 'iplicit' THEN
    -- Location provided but no Iplicit mapping → cannot attribute costs
    ...
    ELSE
      ...
      FROM get_iplicit_pl_amount_cost_by_date(
        ...
        p_end_date,
        'TC',
        v_legal_entity_id  -- NULL = org-wide; set = legal entity filtered
      ) pl;
    END IF;

  ELSIF v_connected_platform = 'xero' THEN
    -- get_xero_op_cost returns 0 gracefully when no accounts are configured.
    IF p_location_id IS NOT NULL THEN
  ...
  ELSIF v_connected_platform = 'quickbooks' THEN
    -- get_quickbooks_op_cost resolves the QB company from the location mapping
    -- internally and sums Expenses-section rows for the date range.
    IF p_location_id IS NOT NULL THEN
      SELECT get_quickbooks_op_cost(p_organization_id, p_location_id, p_start_date, p_end_date)
      INTO v_op_costs;
    ELSE
      SELECT COALESCE(SUM(get_quickbooks_op_cost(p_organization_id, loc.id, p_start_date, p_end_date)), 0)
      INTO v_op_costs
      FROM practice_locations loc
      WHERE loc.organization_id = p_organization_id;
    END IF;
