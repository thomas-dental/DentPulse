-- Add 'In surgery' state to current month working hours calculation.
-- Dentally's "Total hours" includes Completed + Pending + In surgery.
-- Previously we only counted Completed + Pending, causing us to be
-- slightly below Dentally when appointments are actively in progress.
