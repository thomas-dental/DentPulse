-- Fix chart_get_profit_metrics: use net production (private + membership + nhs)
-- instead of total_amount (TPI only).
--
-- Same root cause as 20260602000001 for chart_get_associate_performance_metrics.
...
--
-- Using total_amount caused the Profit chart ranking to calculate P/L from
-- TPI-only production while the Profit Goals Settings used net production,
-- producing different periodic_profit values for the same provider/period.
