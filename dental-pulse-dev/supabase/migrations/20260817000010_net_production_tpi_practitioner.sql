-- ============================================================================
-- Foazia Sikandar Appoline Sep-25: DentPulse £5,648 vs Dentally £5,356.85.
--
-- Dentally Practitioner Activity for "Foazia Sikandar (Appoline Dental Care)"
-- is TPI practitioner 67715 = £5,356.85 (51 items). DentPulse showed
-- £5,647.85 because 00009 attributed production to
-- COALESCE(appointment.practitioner_id, tpi.practitioner_id). Four items
-- (£85+£45+£40+£121 = £291) have TPI practitioner 83516 (Foazia's other
-- site) and appointment clinician 67715, so they were folded onto Appoline
-- Foazia. Dentally keeps them on 83516 — the Practitioner column is the
-- treatment practitioner, not the appointment clinician.
--
-- Same pattern as Zahid's £150 retainer (different person on the
-- appointment): do not steal TPI onto the appointment clinician.
-- Dimitra/Robert mis-stamps still count at the APPOINTMENT SITE via
-- Path 2, but they stay on the TPI practitioner id. Location-scoped
-- grouping in useAllProvidersNetProduction then matches Dentally's
-- per-record filter when a location is selected.
--
-- Rule: practitioner = TPI practitioner; location = appointment site;
-- drop the row when the appointment clinician is a different person.
-- ============================================================================
