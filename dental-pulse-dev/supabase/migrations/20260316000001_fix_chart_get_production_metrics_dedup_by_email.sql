-- Fix chart_get_production_metrics to deduplicate providers registered at multiple locations
-- Groups by email (fallback to name) so a person appearing in 4 location rows
-- is returned as a single row with summed production and hours.
...
BEGIN
  ...
  -- Same person registered at N locations → one row with all their external_ids collected.
  base_providers AS (
    SELECT
      LOWER(COALESCE(NULLIF(TRIM(p.email), ''), p.name)) AS group_key,
      MIN(p.id)   AS provider_id,
  ...
  -- Step 2: Calculate production across all external_ids for the group
  ...
  -- Step 3: Calculate working hours across all external_ids for the group
  provider_hours AS (
    SELECT
      bp.provider_id,
...
'Production metrics deduplicated by provider email (fallback: name).
A provider registered at multiple Dentally locations is merged into one row
with summed production and hours before ranking.';
