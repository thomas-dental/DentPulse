-- ============================================================================
-- Zahid Hussain (Therapist) Jul-26 Appoline showed £2,053 vs Dentally /
-- Setup Categories Private Income £1,902.50.
--
-- Extra £150 is a "Retainer Lower" whose TPI practitioner is St Catherine's
-- Zahid (290685) but the appointment was with a different Appoline
-- practitioner (57217) at Appoline. Email-grouping then folded 290685 into
-- Appoline Zahid (305674).
--
-- Dimitra's £185 items are the opposite: TPI practitioner is St Catherine's
-- Dimitra, appointment practitioner is Appoline Dimitra — those MUST stay
-- on Appoline Dimitra.
--
-- Fix: attribute production to COALESCE(appointment.practitioner_id,
-- treatment_plan_items.practitioner_id). Location stays appointment site.
-- ============================================================================
...
BEGIN
  ...
  -- Practitioner follows the appointment (Dentally), TPI practitioner fallback.
  tpi_home AS (
    SELECT
    ...
    WHERE tpi.organization_id     = p_organization_id
      AND COALESCE(ap.apmt_practitioner_id, tpi.tpi_practitioner_id) IS NOT NULL
