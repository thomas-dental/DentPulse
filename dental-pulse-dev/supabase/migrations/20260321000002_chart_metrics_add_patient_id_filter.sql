-- Add apmt_patient_id IS NOT NULL to chart_get_production_metrics and
-- chart_get_profit_metrics to match Dentally's working hours calculation.
--
-- Root cause of Avg Daily Production mismatch between Overview and Profit Goals:
--   Overview (chart_get_production_metrics):  £2,322.21  — includes null-patient appts
--   Providers/Profit Goals:                   £2,416.17  — excludes null-patient appts
--
-- Dentally's own report filters apmt_patient_id IS NOT NULL, confirmed by:
--   SELECT ... FROM appointments
--   WHERE apmt_patient_id IS NOT NULL AND apmt_state IN ('Completed',...)
--   → 87.1 hrs (matches Dentally exactly)
--
-- All working hours functions now use the same 5-condition standard:
--   states: Completed, Pending, In surgery, Confirmed
--   apmt_duration IS NOT NULL
--   apmt_patient_id IS NOT NULL
--   apmt_start_time IS NOT NULL
--   deleted_at IS NULL

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. chart_get_production_metrics (Overview Ranking chart)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. chart_get_profit_metrics (Profit chart)
-- ─────────────────────────────────────────────────────────────────────────────
