-- get_chair_metrics: utilisation now counts only treatment time tied to
-- actual appointments, not every completed TPI in the period.
--
-- Why: TPIs include charting nomenclatures (Missing tooth, Unerupted, etc.)
-- and other entries that are not chair-time work. Summing all completed
-- TPI durations inflates utilisation. Restricting to TPIs linked to a
-- treatment_appointment row matches how the chair was actually used.
--
-- Logic mirrors the previous occupancy_current CTE from
-- 20260424000003_chair_metrics_match_dentally_duration.sql:
--   apmt_base → treatment_appointments → tpi_dedup → treatments
-- Period is defined by apmt_base (apmt_start_time in [start, end]) — the
-- chair time you're measuring is when the chair was in use.
--
-- tpi_dedup: same dedup key as the prior migration, in case Dentally
-- re-syncs the same TPI under a new id.
...
BEGIN
  ...
  --
  -- Why the cap: treatments.duration_minutes is a *template* (the
  -- typical time the procedure takes), not actual delivered time.
  -- A 25-min appointment can have multiple TPIs whose templates sum
  -- to 90+ min — adding them naively pushes utilisation > 100% (we
  -- saw 574% in production). The chair was booked for the appointment
  -- duration; you cannot deliver more treatment than that. So we sum
  -- per appointment, then take LEAST(template_sum, appointment_hrs).
  --
  -- Side-effect: utilisation_pct is now mathematically ≤ occupancy_pct.
  -- For appointments where templates fit (single-treatment, short
  -- procedure), nothing changes vs. raw template sum. For overflowing
  -- multi-TPI appointments, we treat the chair as fully utilised.
  tpi_current AS (
    SELECT
      capped.loc_id,
